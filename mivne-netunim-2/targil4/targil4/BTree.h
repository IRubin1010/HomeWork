...
public:
	...
	//hoework 
	//search return bool
	bool search(T val) {return searchSubTree(root, val); }
	bool searchSubTree(BNode<T> *t, T val);
	//search that returns pointer to the node of the value
	BNode<T>* searchKey(T val) { return searchKeySubTre(root, val); }
	BNode<T>* searchKeySubTre(BNode<T> *t, T val);
	//hoework
...
template <typename T>
bool BTree<T>::searchSubTree(BNode<T> *t, T val)
{
	if (t == NULL) return false;
	if (t->indexValInK(val) != -1) return true;
	int index = t->indexValGEQInK(val);
	return searchSubTree(t->Son[index], val);
}

//Returns pointer to the node of the value
template<typename T>
BNode<T>* BTree<T>::searchKeySubTre(BNode<T>* t, T val)
{
	if (t == NULL)return NULL;
	if (t->indexValInK(val) != -1) return t;
	int index = t->indexValGEQInK(val);
	return searchKeySubTre(t->Son[index], val);
}
